...
Returns an array of [sticker](#sticker-object) objects for the given guild. Includes the `user` field if the user has the `MANAGE_EXPRESSIONS` permission.

<RouteHeader method="GET" url="/guilds/{guild.id}/stickers/{sticker.id}">
  Get Guild Sticker
</RouteHeader>

Returns a [sticker](#sticker-object) object for the given guild and sticker IDs. Includes the `user` field if the user has the `MANAGE_EXPRESSIONS` permission.

<RouteHeader method="POST" url="/guilds/{guild.id}/stickers" supportsAuditReason>
  Create Guild Sticker
</RouteHeader>

Creates a new sticker for the guild. Must be a `multipart/form-data` body. Requires the `MANAGE_EXPRESSIONS` permission. Returns the new [sticker](#sticker-object) object on success. Fires a [Guild Stickers Update](/topics/gateway-events#guild-stickers-update) Gateway event.
...
Modifies the given sticker. Requires the `MANAGE_EXPRESSIONS` permission. Returns the updated [sticker](#sticker-object) object on success. Fires a [Guild Stickers Update](/topics/gateway-events#guild-stickers-update) Gateway event.
...
Deletes the given sticker. Requires the `MANAGE_EXPRESSIONS` permission. Returns a 204 empty response on success. Fires a [Guild Stickers Update](/topics/gateway-events#guild-stickers-update) Gateway event.
